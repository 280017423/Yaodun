 ...
 * @author zou.sq
 */
public class MyDoctorDetailActivity extends YaodunActivityBase implements OnClickListener {
			...
			if (result != null) {
				switch (msg.what) {
					case SEND_DATA_SUCCESSED:
						...
						toast("评论成功");
						ImeUtil.hideSoftInput(MyDoctorDetailActivity.this);
						break;
					case SEND_DATA_FAIL:
	...
	private void initView() {
		...
		mTvnRight.setText(R.string.knowledge_detail_cancel_attention);
	...
	private void sendReplay() {
		...
		mIsSend = true;
		new ActionProcessor().startAction(MyDoctorDetailActivity.this, new IActionListener() {
	...
	private void attentionKnowledge() {
		...
		new ActionProcessor().startAction(MyDoctorDetailActivity.this, new IActionListener() {
			...
			@Override
			public ActionResult onAsyncRun() {
				return DoctorReq.attentionDoctor(mDoctorModel.getDoctorId(), "1");
			}
		});
	}
